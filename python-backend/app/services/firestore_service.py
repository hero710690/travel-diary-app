...
class FirestoreTripService:
    ...
    def _convert_trip_data(self, trip_dict: Dict[str, Any]) -> Dict[str, Any]:
        """Convert Firestore trip data to Trip model format (snake_case)"""
        converted = {
            'id': trip_dict.get('id'),
            ...
            'total_budget': trip_dict.get('total_budget', 0),
    ...
    async def get_trip_by_id(self, trip_id: str, user_id: str, user_email: str = '') -> Optional[Trip]:
        try:
            trip_data = await self.db.get_trip_by_id(trip_id, user_id, user_email)
            if trip_data:
